...
int fifteen_getmove (Pos *pos, int x, int y, int type, Player, byte **);
...
int fifteen_getmove_kb (Pos *pos, int key, Player glob_to_play, byte **movp)
{
	static byte move[10];
	byte *mp = move;
	int i, j, incx, incy;
...
int fifteen_getmove (Pos *pos, int x, int y, int type, Player to_play, 
		byte **movp)
{
	int k;
	static byte move[16];
